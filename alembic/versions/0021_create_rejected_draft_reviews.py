...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("workflow_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("lead_workflows.workflow_id"), nullable=False),
        sa.Column("workflow_transition_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("workflow_transitions.transition_id"), nullable=False),
        sa.Column("campaign_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("campaigns.campaign_id"), nullable=False),
        sa.Column("campaign_version_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("campaign_versions.campaign_version_id"), nullable=False),
        sa.Column("cadence_step_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("campaign_cadence_steps.cadence_step_id"), nullable=False),
        ...
        sa.Column("draft_personalization_notes", postgresql.JSONB(astext_type=sa.Text()), nullable=False),
        ...
        sa.Column("reviewed_by_user_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.user_id"), nullable=True),
        sa.Column("reviewed_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column("review_note", sa.String(), nullable=True),
        sa.Column("outbound_message_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("outbound_messages.message_id"), nullable=True),
