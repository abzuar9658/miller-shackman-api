...
from app.application.services.crm_lead_refresh import (
    CrmTagEnrollmentDependencies,
    run_crm_tag_enrollment,
)
...
async def handle_people_event(
    ...
    for person in people:
        ...
        if event_type != "peopleDeleted":
            enrollment_result = await run_crm_tag_enrollment(
                workspace_id=workspace_id,
                lead=saved,
                observed_at=occurred_at,
                now=now,
                deps=crm_tag_enrollment_dependencies(bundle),
            )
...
def crm_tag_enrollment_dependencies(
    bundle: FollowUpBossWebhookEventBundle,
) -> CrmTagEnrollmentDependencies:
    return CrmTagEnrollmentDependencies(
        ...
        workspace_operational_control_repository=(
            bundle.workspace_operational_control_repository
        ),
